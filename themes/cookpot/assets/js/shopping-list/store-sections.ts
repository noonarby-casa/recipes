...
export const DAIRY_FIRST_SECTIONS: StoreSection[] = STANDARD_SECTIONS.map(
  (sec) => {
    let order = sec.order;
    if (sec.id === "dairy") order = 1;
    else if (sec.id === "produce") order = 4; // swap dairy and produce
    return { ...sec, order };
  },
);
...
export const MEAT_FIRST_SECTIONS: StoreSection[] = STANDARD_SECTIONS.map(
  (sec) => {
    let order = sec.order;
    if (sec.id === "meat") order = 1;
    else if (sec.id === "produce") order = 3; // swap meat and produce
    return { ...sec, order };
  },
);
