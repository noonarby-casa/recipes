...
- **Supported Units:** The scaling engine in [scaler.ts](themes/cookpot/assets/js/scaler.ts) parses units defined in [constants.ts](themes/cookpot/assets/js/constants.ts). Supported units and standard abbreviations include: `ounces` (`ounces`, `ounce`, `oz`), `pounds` (`pounds`, `pound`, `lb`, `lbs`), `cups` (`cups`, `cup`), `teaspoons` (`teaspoons`, `teaspoon`, `tsp`), `tablespoons` (`tablespoons`, `tablespoon`, `tbsp`), `cloves` (`cloves`, `clove`), `cans` (`cans`, `can`), `grams` (`grams`, `gram`, `g`), `ml`, `small`, `large`, `medium`, `heads` (`heads`, `head`), `bulbs` (`bulbs`, `bulb`), `bunches` (`bunch`, `bunches`), `strips` (`strip`, `strips`), `ears` (`ear`, `ears`), `loaves` (`loaf`, `loaves`), `slices` (`slice`, `slices`), `ribs` (`rib`, `ribs`), and `bags` (`bag`, `bags`).
- **Ingredient Naming:** Ingredient names in the recipe TOML front matter should be specified in the singular form (e.g., `item = "egg"`, `item = "chicken breast"`, `item = "grape tomato"`). The client-side engine automatically handles pluralization and singularization on display (e.g. `3 large eggs`, `1 chicken breast`, `1 package grape tomatoes`).
- **Usage Rules:**
